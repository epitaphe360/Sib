...
class PushNotificationService {
  ...
  private handleForegroundMessage(payload: FCMMessage) {
    ...
    // Create notification object
      ...
      title: notification?.title || 'sib 2026',
    ...
    window.dispatchEvent(
      new CustomEvent('sib:push-notification', { detail: notificationData })
    );
  }
  ...
  onNotificationReceived(callback: (notification: NotificationData) => void) {
    window.addEventListener('sib:push-notification', (event: CustomEvent<NotificationData>) => {
