...
function exportCsv(rows: ScannedVisitor[], sponsorName: string, scannerName?: string) {
  ...
  a.href = url;
  const safeName = sponsorName.replaceAll(' ', '_');
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors">
      ...
      <td className="py-3 px-4 hidden sm:table-cell">
        <span className="text-sm text-gray-600">{scan.visitor_company ?? '—'}</span>
      </td>
...
export default function PartnerScansPage() {
  ...
  const fetchScans = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      // Collaborateurs actifs du sponsor (owner_type = 'partner')
  ...
  const sponsorName = (user?.profile as { organization?: string; company?: string } | undefined)?.organization
    ?? (user?.profile as { organization?: string; company?: string } | undefined)?.company
    ?? user?.name ?? 'sponsor';
