...
const STATUS_CONFIG: Record<InvitationStatus, { label: string; color: string; Icon: React.FC<{className?: string}> }> = {
  pending: { label: 'En attente', color: 'bg-amber-50 text-amber-700 border-amber-200', Icon: Clock },
  approved: { label: 'Approuvée', color: 'bg-green-50 text-green-700 border-green-200', Icon: CheckCircle },
...
export default function InvitationLetterPage({ userType }: InvitationLetterPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.visit_dates.length === 0) {
      toast.error('Sélectionnez au moins une date de visite');
      return;
    }
    ...
    try {
      ...
      if (error) {throw error;}
      toast.success('Demande envoyée ! L\'administrateur traitera votre demande sous 48h.');
      setForm(emptyForm);
      setShowForm(false);
      fetchLetters();
    } catch (err: any) {
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Lettres d'Invitation</h1>
                <p className="text-sm text-gray-500">Demandez une lettre d'invitation officielle pour vos visiteurs</p>
              </div>
            </div>
            <button
        ...
        <AnimatePresence>
          {showForm && (
            ...
            >
              <h2 className="text-lg font-bold text-gray-900 border-b pb-4">Informations du destinataire</h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Date d'expiration passeport *</label>
                ...
                {/* Société & Fonction */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Société / Organisme *</label>
              ...
              <h2 className="text-lg font-bold text-gray-900 border-b pb-4 pt-2">Informations de visite</h2>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea required value={form.visit_purpose}
                    ...
                    rows={3}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm"
                    placeholder="Participation au Salon International du Bâtiment SIB 2026 en tant que visiteur professionnel..." />
                </div>

                {/* Dates */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Date(s) de visite souhaitée(s) *</label>
                ...
                {/* Langue */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Langue de la lettre</label>
                ...
                {exhibitorData && (
                  <div className="bg-gray-50 rounded-lg p-4 text-sm text-gray-600">
                    <strong>De la part de :</strong> {exhibitorData.company_name}
                    {exhibitorData.stand_number && ` — Stand ${exhibitorData.stand_number}`}
                  </div>
                )}
              </div>
        ...
        <div>
          <h2 className="text-base font-bold text-gray-900 mb-4">Mes demandes</h2>
          {isLoading ? (
            ...
            <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-10 text-center">
              ...
              <p className="text-gray-500 text-sm">Aucune demande de lettre d'invitation</p>
            </div>
          ) : (
                  ...
                  <div key={letter.id} className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                    <div className="flex items-start justify-between flex-wrap gap-3">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-2 text-xs text-gray-400">
                          ...
                          {letter.visit_dates.map(d => SALON_DATES.find(s => s.value === d)?.label).join(', ')}
                    ...
                    {letter.admin_note && letter.status === 'rejected' && (
                      <div className="mt-3 bg-red-50 border border-red-100 rounded-lg p-3 text-sm text-red-700">
                        <strong>Motif du refus :</strong> {letter.admin_note}
                      </div>
                    )}
                    <div className="text-xs text-gray-400 mt-3">
                      Demandé le {new Date(letter.created_at).toLocaleDateString('fr-FR')}
                    </div>
                  </div>
                );
              })}
