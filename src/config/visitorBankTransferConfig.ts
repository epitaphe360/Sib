...
export const VISITOR_BANK_TRANSFER_INFO = {
  ...
  // Métadonnées Pass VIP (montant : voir visitor_levels via visitorLevelService)
  vipPass: {
    currency: 'EUR',
...
/**
 * Instructions de virement avec montant dynamique (config admin visitor_levels).
 */
export function getVisitorBankTransferInstructions(
  ...
  const byLang = {
    fr: {
      ...
      steps: [
        `Effectuez un virement bancaire de ${formatted}`,
      ...
      important: [
        ...
        `⚠️ Le montant doit être exactement ${formatted}`,
    ...
    en: {
      ...
      steps: [
        `Make a bank transfer of ${formatted}`,
      ...
      important: [
        ...
        `⚠️ Amount must be exactly ${formatted}`,
    ...
    ar: {
      ...
      steps: [
        `قم بتحويل بنكي بقيمة ${formatted}`,
      ...
      important: [
        ...
        `⚠️ يجب أن يكون المبلغ ${formatted} بالضبط`,
