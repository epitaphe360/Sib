  ...
  const getBotResponse = (userMessage: string): ChatMessage => {
    ...
    if (!isAuthenticated) {
      ...
      if (message.includes('exposant') || message.includes('entreprise')) {
        return {
          id: Date.now().toString(),
          content: "?? SIB 2026 accueille 300+ exposants internationaux ! Découvrez les leaders de l'industrie portuaire. Connectez-vous pour accéder au réseautage intelligent et aux RDV B2B.",
          isBot: true,
          timestamp,
          type: 'suggestion',
          suggestions: [
      ...
      if (message.includes('salon') || message.includes('SIB') || message.includes('information')) {
        return {
          id: Date.now().toString(),
          content: "?? SIB 2026 - Le plus grand salon portuaire international ! ?? 1-3 Avril 2026 à Casablanca, Maroc. 300+ exposants, 6,000+ visiteurs de 40 pays, 40+ conférences !",
          isBot: true,
          timestamp,
          type: 'quick_reply',
    ...
    if (message.includes('salon') || message.includes('SIB') || message.includes('information')) {
      return {
        id: Date.now().toString(),
        content: `?? Bonjour ${firstName} ! SIB 2026 se déroule du 1er au 3 avril 2026 à Casablanca, Maroc. C'est le plus grand salon portuaire international avec 300+ exposants, 6,000+ visiteurs de 40 pays !`,
        isBot: true,
        timestamp,
        type: 'suggestion',
        suggestions: [
