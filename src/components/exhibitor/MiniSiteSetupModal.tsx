  ...
  const handleAutoCreate = async () => {
    ...
    try {
      ...
      if (!import.meta.env.VITE_OPENAI_API_KEY) {
        toast.info(
          '?? Configuration requise : La clé API OpenAI doit être ajoutée dans le fichier .env',
          { duration: 5000 }
        );
      ...
      if (error?.message?.includes('API key')) {
        toast.error(
          '?? Configuration requise : Veuillez ajouter la clé API OpenAI.',
          { duration: 6000 }
        );
      } else if (error?.message?.includes('CORS') || error?.message?.includes('Failed to fetch')) {
        toast.error(
          '?? Impossible d\'accéder au site web. Vérifiez l\'URL ou utilisez la création manuelle.',
          { duration: 6000 }
        );
      } else {
        toast.error(
