  ...
  const handleCreateSlot = async () => {
    try {
      ...
      if (!isDateInSalonRange(selectedDate)) {
        toast.error(`La date doit être comprise entre le ${DEFAULT_SALON_CONFIG.dates.start} et le ${DEFAULT_SALON_CONFIG.dates.end}`);
        return;
      }
  ...
  const handleBookSlotImproved = async () => {
    ...
    try {
      ...
      if (!isExhibitorOrPartner) {
        // Pré-vérification du quota côté client (visiteurs uniquement)
              ...
              >
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Message pour l'exposant (optionnel)
                    </label>
                    <textarea
                      value={bookingMessage}
                      onChange={(e) => setBookingMessage(e.target.value)}
                      placeholder="Présentez-vous et expliquez l'objet de votre demande..."
                ...
                <div className="flex justify-end space-x-3 mt-6">
                  ...
                  <Button
                    ...
                    disabled={isBooking}
                  >
                    {isBooking ? 'Envoi en cours...' : 'Envoyer la demande'}
                  </Button>
                </div>
              </motion.div>
            </div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <div className="flex items-center space-x-2 text-blue-800">
                      ...
                      <span className="text-sm font-medium">
                        Créneaux uniquement du {DEFAULT_SALON_CONFIG.dates.start} au {DEFAULT_SALON_CONFIG.dates.end}
                      </span>
                    </div>
                  </div>
