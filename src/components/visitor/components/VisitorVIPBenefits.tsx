...
export function VisitorVIPBenefits() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-8">
      <Card className="p-6 bg-[#0F2034] border border-[#F39200]/20 rounded-xl shadow-lg relative overflow-hidden">
        <div className="absolute top-0 right-0 p-4 opacity-5">
          <Crown className="w-32 h-32 text-[#F39200]" />
        </div>
        <div className="flex items-center gap-3 mb-6 relative z-10">
          <div className="p-2 bg-[#F39200]/15 rounded-lg border border-[#F39200]/30">
            <Crown className="h-6 w-6 text-[#F39200]" />
          </div>
          <div>
            <h3 className="text-xl font-bold text-white">{t('visitor.vip_benefits_title')}</h3>
            <p className="text-[#F39200]/80 text-sm">{t('visitor.vip_benefits_subtitle')}</p>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 relative z-10">
          {[
            ...
            const cls = "flex items-start gap-3 p-3 bg-white/5 rounded-lg border border-[#F39200]/15 transition-all hover:bg-white/10 hover:border-[#F39200]/30";
            const inner = (
              <>
                <Icon className="h-5 w-5 text-[#F39200] mt-0.5" />
                <div>
