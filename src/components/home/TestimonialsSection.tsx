...
const TestimonialCard: React.FC<TestimonialCardProps> = ({
  ...
  return (
    <motion.div
      whileHover={{ y: -8 }}
      className="relative bg-white/90 backdrop-blur-md rounded-2xl p-8 border-l-4 border-primary-500 shadow-lg hover:shadow-xl transition-all h-full"
    >
      ...
      <div className="relative z-10">
          ...
          <div className="flex gap-1 mb-4">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className={`w-4 h-4 ${
                  i < rating ? 'text-primary-500 fill-primary-500' : 'text-slate-300'
                }`}
              />
            ))}
          </div>
        ...
        <div className="flex items-center gap-4 pt-6 border-t border-slate-200">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-bold text-indigo-600 text-lg truncate">
              {name}
            </div>
            {position && (
...
export const TestimonialsSection: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-indigo-600 via-indigo-500 to-sib-accent relative overflow-hidden">
