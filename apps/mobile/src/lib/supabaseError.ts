export function supabaseErrorMessage(error: unknown, fallback: string): string {
  if (error && typeof error === 'object') {
    ...
    if (code === '23505') {
      ...
      if (details.includes('idx_stand_collaborators_email') || message.includes('idx_stand_collaborators_email')) {
        return `Cet email est déjà utilisé par un collaborateur`;
      }
