...
export function QRBadgeView({ badge, qrValue, fullScreen }: { badge: UserBadge; qrValue: string; fullScreen?: boolean }) {
  ...
  const eventTitle = activeSalon
    ? (theme?.fullName ?? activeSalon.name)
    : PLATFORM.name;
  const eventMeta = activeSalon
    ? `${activeSalon.dates ?? config.event_dates_display} · ${theme?.location ?? config.event_location}`
    : t('badge.globalMeta');
  ...
  return (
    <View style={[styles.wrap, fullScreen && styles.wrapFull]}>
      <View style={[styles.pass, shadows.lg]}>
        ...
        {/* QR body — sans chevauchement agressif sur le header */}
