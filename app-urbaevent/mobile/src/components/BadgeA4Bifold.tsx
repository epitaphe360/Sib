...
type Props = {
  ...
  /** QR participant (sécurisé ou statique) — même payload que le scan à l'entrée */
  qrValue?: string;
  /** Masque légendes (capture impression) */
  printMode?: boolean;
};
...
function FaceInfosPratiques({ config }: { config: BadgeConfig; primary: string }) {
  const locationQr = resolveBadgeAssetUrl(config.location_qr_url);
  return (
    <View style={styles.faceCol}>
      ...
      <View style={styles.infoList}>
        ...
        <View style={styles.infoRow}>
          ...
          <View style={styles.infoContent}>
            <Text style={styles.infoTitle}>VENIR AU SALON</Text>
            <Text style={styles.infoValue}>{config.how_to_get_there}</Text>
          </View>
          {locationQr ? (
            <View style={styles.locationQrWrap}>
              <Image source={{ uri: locationQr }} style={styles.locationQr} resizeMode="contain" />
              <Text style={styles.locationQrHint}>Scannez ici{'\n'}pour la localisation</Text>
            </View>
          ) : null}
        </View>
        <InfoRow
...
function FaceImagePleine({ config }: { config: BadgeConfig }) {
  const promoImg = resolveBadgeAssetUrl(config.app_promo_image_url);
  if (promoImg) {
    return <Image source={{ uri: promoImg }} style={styles.fullImage} resizeMode="cover" />;
  }
  return (
    <View style={[styles.faceCol, styles.imageFallback]}>
      <Text style={styles.imageFallbackText}>{config.event_name}</Text>
    </View>
  );
}
...
function FacePartenaires({ config, primary }: { config: BadgeConfig; primary: string }) {
  ...
  const logoHeight = Math.max(18, Math.min(40, (config.partners_logo_height ?? 13) * 2.2));
            ...
            {row.map((item) => (
              <View key={item.key} style={styles.logoCell}>
                {resolveBadgeAssetUrl(item.url) ? (
                  <Image
                    source={{ uri: resolveBadgeAssetUrl(item.url)! }}
                    style={[styles.logoImg, { height: logoHeight }]}
                    resizeMode="contain"
                  />
                ) : (
...
function FaceProgramme({ config, primary, secondary }: { config: BadgeConfig; primary: string; secondary: string }) {
  const compact = config.program_compact_mode ?? 'false';
  const maxSessions = compact === 'false' ? 3 : 999;
  const sessionFont = compact === 'mini' ? 5 : compact === 'small' ? 5.5 : 6;
  const dayFont = compact === 'mini' ? 6 : compact === 'small' ? 6.5 : 7;
    ...
    <View style={styles.faceCol}>
      ...
      {config.show_program_on_badge === 'true' && config.program_days.length > 0 ? (
        <View style={styles.programBody}>
          {compact === 'false' ? (
            <Text style={[styles.programTitle, { color: primary }]}>Programme {config.event_name}</Text>
          ) : null}
          {config.program_days.map((programDay) => (
            <View key={programDay.id} style={styles.programDayBlock}>
              <Text style={[styles.programDayLabel, { backgroundColor: secondary, fontSize: dayFont }]}>
                {programDay.label}
              </Text>
              {programDay.sessions.slice(0, maxSessions).map((s, i) => (
                <Text key={`${programDay.id}-${i}`} style={[styles.sessionLine, { fontSize: sessionFont }]}>
                  <Text style={{ fontFamily: fonts.bodyBold }}>{s.time}</Text> · {s.title}
                </Text>
              ))}
              {compact === 'false' && programDay.sessions.length > 3 ? (
                <Text style={styles.programMore}>+{programDay.sessions.length - 3} sessions</Text>
              ) : null}
            </View>
          ))}
        </View>
...
function renderFace(
  ...
  if (content === 'image_pleine') {
    return (
      <View style={styles.quadrant}>
        <FaceImagePleine config={config} />
      </View>
    );
  }
...
export const BadgeA4Bifold = forwardRef<ScrollViewType, Props>(function BadgeA4Bifold(
  { badge, previewScale = true, qrValue: qrValueProp, printMode = false },
  ref,
) {
  const { config, loading } = useBadgeConfig();
  const participantQr = qrValueProp ?? buildStaticParticipantQR(badge);
  ...
  if (loading) {
    return (
      <View style={styles.loadingWrap}>
        ...
        <Text style={styles.loadingText}>Chargement du modèle badge admin…</Text>
      </View>
    );
  }
    ...
    <ScrollView ref={ref} collapsable={false} contentContainerStyle={styles.scrollOuter}>
      <View style={[styles.sheet, { width: sheetW, height: sheetH }]}>
        <View style={styles.grid}>
          {renderFace(config.face4_content, config, badge, participantQr, primary, secondary)}
          {renderFace(config.face1_content, config, badge, participantQr, primary, secondary)}
          {renderFace(config.face2_content, config, badge, participantQr, primary, secondary)}
          {renderFace(config.face3_content, config, badge, participantQr, primary, secondary)}
        </View>
      ...
      {!printMode ? (
        <>
          ...
          <Text style={styles.configHint}>Modèle synchronisé avec /admin/badge-config</Text>
        </>
      ) : null}
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  programTitle: { fontFamily: fonts.bodyBold, fontSize: 8, marginBottom: 4, fontStyle: 'italic' },
  programDayBlock: { marginBottom: 4 },
  programDayLabel: {
    color: '#fff',
  ...
  programMore: { fontFamily: fonts.body, fontSize: 5.5, color: '#999', paddingLeft: 4 },
  ...
  imageFallbackText: { fontFamily: fonts.bodyBold, fontSize: 10, color: '#6b7280' },
  locationQrWrap: { alignItems: 'center', marginLeft: 4 },
  locationQr: { width: 36, height: 36, backgroundColor: '#fff', borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 3 },
  locationQrHint: { fontSize: 4.5, color: '#9ca3af', textAlign: 'center', marginTop: 2, lineHeight: 6 },
  loadingWrap: { alignItems: 'center', justifyContent: 'center', paddingVertical: spacing.xl, gap: spacing.sm },
  loadingText: { fontFamily: fonts.body, fontSize: 12, color: '#6b7280', textAlign: 'center' },
  caption: {
