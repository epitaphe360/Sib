...
describe('Mobile — qualité statique', () => {
  describe('versions et identité', () => {
    it('app.json et package.json alignés sur 1.0.19', () => {
      const appJson = JSON.parse(read('app.json'));
      const pkg = JSON.parse(read('package.json'));
      expect(appJson.expo.version).toBe('1.0.19');
  ...
  describe('architecture UrbaEvent hub → salon', () => {
    ...
    it('plan interactif 3D avec stands', () => {
      expect(read('src/components/InteractiveFloorPlan.tsx')).toContain('computeStandPins');
      expect(existsSync(join(MOBILE_ROOT, 'src/lib/floorPlanStandPins.ts'))).toBe(true);
    });
