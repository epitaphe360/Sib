    ...
    try {
      ...
      for (const [key, value] of Object.entries(parsed)) {
        if (typeof value !== 'string') {
          throw new Error(`La clé "${key}" doit être une chaîne (ex. "8000+" et non 8000)`);
        }
      }
    } catch (err) {
          ...
          {activeSlug === 'programme_scientifique' && (
            <>
              <p className="w-full text-xs text-amber-700 dark:text-amber-300">
                Ne modifiez pas <code className="font-mono">programme_data</code> ici — utilisez l’éditeur structuré. Les textes title/subtitle/intro peuvent être édités ci-dessous.
              </p>
              <a
                href="/admin/events"
