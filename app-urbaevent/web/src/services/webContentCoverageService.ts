...
import { fetchMobileAppContent, DEFAULT_MOBILE_APP_CONTENT, type MobileAppContent } from './mobileAppContentService';
...
export async function computeWebContentCoverage(): Promise<WebContentCoverage> {
  const [images, texts, pages, mobileResult] = await Promise.all([
    fetchSiteImages(),
    fetchSiteTextContent(),
    getAllPageMeta(),
    fetchMobileAppContent().then((content) => ({ ok: true as const, content })).catch(() => ({
      ok: false as const,
      content: DEFAULT_MOBILE_APP_CONTENT,
    })),
  ]);
  const mobile = mobileResult.content;
