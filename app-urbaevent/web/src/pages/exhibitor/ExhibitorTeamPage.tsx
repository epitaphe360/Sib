...
export default function ExhibitorTeamPage({ userType = 'exhibitor' }: ExhibitorTeamPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authError) {
        throw new Error(authError.message || t('team.create_error'));
      }
      if (authData?.error) {
        throw new Error(String(authData.error));
      }

      const authUserId: string | null = authData?.user_id || null;
      if (!authUserId) {
        throw new Error(t('team.create_error'));
      }

      // 3. Insérer le collaborateur en base (trigger DB génère le badge auto)
      const { data: inserted, error: insertError } = await (supabase as any)
        ...
        .select('id, badge_generated')
        .single();

      if (insertError) {throw insertError;}

      // Secours si trigger pas encore déployé
      if (!inserted?.badge_generated) {
        const { error: badgeError } = await (supabase as any).rpc('upsert_user_badge', {
          p_user_id: authUserId,
          p_user_type: 'exhibitor',
          p_full_name: `${form.first_name} ${form.last_name}`.trim(),
          p_company_name: companyName,
          p_position: form.position || 'Collaborateur',
          p_email: emailLower,
          p_phone: form.phone || null,
          p_stand_number: standNumber || null,
        });
        if (badgeError) {throw badgeError;}
        await (supabase as any)
          .from('stand_collaborators')
          .update({ badge_generated: true })
          .eq('id', inserted.id);
      }
