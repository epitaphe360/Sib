...
-- Look up partners.id by user_id since partner_id is FK to partners(id)
...
DECLARE
  v_partner_id UUID;
BEGIN
  -- Projects for Platinium Global Corp (user_id 00000000-0000-0000-0000-000000000107)
  SELECT id INTO v_partner_id FROM partners WHERE user_id = '00000000-0000-0000-0000-000000000107' LIMIT 1;
  IF v_partner_id IS NOT NULL THEN
    ...
    VALUES (
      ...
      'active', '2024-01-01', '2026-12-31', '15.5M €', 'Augmentation productivité +40%',
      'https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?w=800&q=80',
      ARRAY['5G', 'IA', 'Edge Computing'], ARRAY['Jean-Marc L.', 'Sarah B.', 'Kevin T.'],
      ...
      ARRAY['Port de Tanger', 'Telecom Global'], '[{"name": "Livre Blanc", "type": "PDF", "url": "#"}]',
      ARRAY['https://images.unsplash.com/photo-1494412519320-aa613dfb7738?w=400']
    );
  END IF;

  -- Projects for Global Shipping Alliance (user_id 00000000-0000-0000-0000-000000000105)
  SELECT id INTO v_partner_id FROM partners WHERE user_id = '00000000-0000-0000-0000-000000000105' LIMIT 1;
  IF v_partner_id IS NOT NULL THEN
    INSERT INTO partner_projects (partner_id, name, description, status, start_date, end_date, budget, impact, image_url, technologies, team, kpis, timeline, project_partners, documents, gallery)
    VALUES (
      v_partner_id,
      ...
      'completed', '2023-05-10', '2024-11-20', '4.2M €', 'Réduction CO2 -25%',
      'https://images.unsplash.com/photo-1578575437130-527eed3abbec?w=800&q=80',
      ARRAY['Big Data', 'Algorithmes Verts'], ARRAY['Marc D.', 'Elena S.'],
      ...
      ARRAY['Green Freight', 'LogiTrack'], '[{"name": "Rapport Impact", "type": "PDF", "url": "#"}]',
      ARRAY['https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=400']
    );
  END IF;

  -- Projects for Port Tech Systems (user_id 00000000-0000-0000-0000-000000000109)
  SELECT id INTO v_partner_id FROM partners WHERE user_id = '00000000-0000-0000-0000-000000000109' LIMIT 1;
  IF v_partner_id IS NOT NULL THEN
    INSERT INTO partner_projects (partner_id, name, description, status, start_date, end_date, budget, impact, image_url, technologies, team, kpis, timeline, project_partners, documents, gallery)
    VALUES (
      v_partner_id,
      ...
      'active', '2024-08-15', '2025-12-01', '2.8M €', 'Sécurité accrue 100%',
      'https://images.unsplash.com/photo-1563986768609-322da13575f3?w=800&q=80',
      ARRAY['Blockchain', 'Hyperledger', 'IoT'], ARRAY['Thomas W.', 'Li Na'],
      '{"progress": 45, "satisfaction": 92, "roi": 120}',
      '[{"phase": "MVP", "date": "2024-12-01", "status": "completed", "description": "Test sur le port de Rotterdam"}]',
      ARRAY['IBM Logistics', 'Maersk'], '[{"name": "Spécifications", "type": "PDF", "url": "#"}]',
      ARRAY['https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=400']
    );
  END IF;

  -- Projects for Museum Heritage (user_id 00000000-0000-0000-0000-000000000108)
  SELECT id INTO v_partner_id FROM partners WHERE user_id = '00000000-0000-0000-0000-000000000108' LIMIT 1;
  IF v_partner_id IS NOT NULL THEN
    INSERT INTO partner_projects (partner_id, name, description, status, start_date, end_date, budget, impact, image_url, technologies, team, kpis, timeline, project_partners, documents, gallery)
    VALUES (
      v_partner_id,
      ...
      'planned', '2025-03-01', '2027-03-01', '1.5M €', 'Éducation & Culture',
      'https://images.unsplash.com/photo-1501503060445-738213995a8c?w=800&q=80',
      ARRAY['Photogrammétrie', 'VR/AR'], ARRAY['Pr. Aris T.', 'Sophie M.'],
      '{"progress": 5, "satisfaction": 100, "roi": 0}',
      '[{"phase": "Scan", "date": "2025-04-01", "status": "upcoming", "description": "Première expédition sous-marine"}]',
      ARRAY['UNESCO', 'DeepSea Research'], '[{"name": "Brochure", "type": "PDF", "url": "#"}]',
      ARRAY['https://images.unsplash.com/photo-1544551763-46a013bb70d5?w=400']
    );
  END IF;

  -- Projects for Silver Tech Group (user_id 00000000-0000-0000-0000-000000000106)
  SELECT id INTO v_partner_id FROM partners WHERE user_id = '00000000-0000-0000-0000-000000000106' LIMIT 1;
  IF v_partner_id IS NOT NULL THEN
    INSERT INTO partner_projects (partner_id, name, description, status, start_date, end_date, budget, impact, image_url, technologies, team, kpis, timeline, project_partners, documents, gallery)
    VALUES (
      v_partner_id,
      ...
      'completed', '2024-01-10', '2024-09-30', '850k €', 'Engagement utilisateur +60%',
      'https://images.unsplash.com/photo-1512428559083-a40ce12b26f0?w=800&q=80',
      ARRAY['React Native', 'Node.js', 'WebSockets'], ARRAY['Alice R.', 'Bob S.'],
      ...
      ARRAY['Expo Global'], '[{"name": "User Guide", "type": "PDF", "url": "#"}]',
      ARRAY['https://images.unsplash.com/photo-1522199755839-a2bacb67c546?w=400']
    );
  END IF;

  -- Projects for Ocean Freight Services (user_id 00000000-0000-0000-0000-000000000110)
  SELECT id INTO v_partner_id FROM partners WHERE user_id = '00000000-0000-0000-0000-000000000110' LIMIT 1;
  IF v_partner_id IS NOT NULL THEN
    INSERT INTO partner_projects (partner_id, name, description, status, start_date, end_date, budget, impact, image_url, technologies, team, kpis, timeline, project_partners, documents, gallery)
    VALUES (
      v_partner_id,
      ...
      'active', '2024-11-01', '2025-12-31', '3.5M €', 'Économie carburant 12%',
      'https://images.unsplash.com/photo-1494412519320-aa613dfb7738?w=800&q=80',
      ARRAY['Python', 'Machine Learning', 'Satellite Data'], ARRAY['Capt. Nemo', 'Dr. Aronnax'],
      ...
      ARRAY['WeatherGlobal'], '[{"name": "Technical Specs", "type": "PDF", "url": "#"}]',
      ARRAY['https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=400']
    );
  END IF;
END $$;
