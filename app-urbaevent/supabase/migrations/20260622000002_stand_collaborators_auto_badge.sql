-- ============================================================
-- Collaborateurs stand : réparation + badge auto à la création
-- Copier TOUT ce fichier dans Supabase SQL Editor puis Run
-- ============================================================
...
BEGIN
  ...
  IF sc.auth_user_id IS NULL THEN
    RETURN;
  END IF;
...
BEGIN
  SELECT * INTO sc FROM public.stand_collaborators WHERE id = p_collaborator_id;
  IF NOT FOUND OR sc.auth_user_id IS NULL THEN
    RETURN false;
  END IF;

  PERFORM public.ensure_collaborator_user_profile(p_collaborator_id);

  SELECT e.company_name, e.stand_number
  INTO v_company, v_stand
  FROM public.exhibitors e
  WHERE e.id = sc.exhibitor_id;

  IF v_company IS NULL THEN
    SELECT p.company_name, NULL
    INTO v_company, v_stand
    FROM public.partners p
    WHERE p.id = sc.partner_id;
  END IF;
  ...
  PERFORM public.upsert_user_badge(
    ...
    trim(sc.first_name || ' ' || sc.last_name),
    v_company,
    COALESCE(sc.position, 'Collaborateur'),
    lower(sc.email),
    sc.phone,
    NULL,
    v_stand
...
RETURNS TABLE(
  out_email text,
  out_auth_linked boolean,
  out_badge_ok boolean,
  out_detail text
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FOR r IN
    SELECT * FROM public.stand_collaborators
    WHERE status = 'active'
    ORDER BY created_at
  LOOP
    ...
    IF NOT v_auth_linked THEN
      SELECT EXISTS(
        SELECT 1 FROM auth.users au WHERE lower(au.email) = lower(r.email)
      ) INTO v_auth_linked;

      IF v_auth_linked THEN
        PERFORM public.sync_collaborator_auth_link(r.email);
        SELECT sc.auth_user_id IS NOT NULL
        INTO v_auth_linked
        FROM public.stand_collaborators sc
        WHERE sc.id = r.id;
      END IF;
    END IF;
    ...
    ELSE
      ...
      out_email       := r.email;
      out_auth_linked := true;
      out_badge_ok    := v_ok;
      out_detail      := CASE
        WHEN v_ok THEN 'OK - profil + badge generes'
        ELSE 'Echec generation badge'
      END;
      RETURN NEXT;
    END IF;
  END LOOP;
...
BEGIN
  IF NEW.auth_user_id IS NOT NULL
     ...
     AND (
       ...
       OR NEW.badge_generated IS NOT TRUE
     )
  THEN
